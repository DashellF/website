...
const SectionsBottom = P(
    ...
    "<p>I've also been doing ctfs every week for over a year now (My solo account is <a class=\"link\" href=\"https://ctftime.org/team/419145\" target=\"_blank\" rel=\"noopener noreferrer\">JAGIC</a> and I do ctfs with <a class=\"link\" href=\"https://ctftime.org/team/220336\" target=\"_blank\" rel=\"noopener noreferrer\">L3AK</a>.)</p>" +
    "</div></div>" +
    '<div class="main-block"><div class="text-block">' +
    "<h2>Skills</h2>" +
    "<p>For ctfs, I usually do really well in <strong>rev</strong>, <strong>osint</strong>, and <strong>misc</strong>, and I can hold my own in <strong>forensics</strong> and <strong>hardware</strong>. I've just started competing with L3AK, but I used to do ctfs solo.</p>" +
...
const Xe = k({
    ...
    setup() {
        ...
        const applyRotatorFallback = () => {
            ...
            const scene = (rot && rot.parentElement) || document.querySelector(".scene");
            if (!rot || !scene) return;
        ...
        const goWriteups = (w) => {
            ...
            if (ws && typeof ws.scrollTop === "number") {
                ws.scrollTop = 0;
                if (typeof ws.scrollTo === "function") ws.scrollTo({ top: 0, behavior: "auto" });
        ...
        D(() => {
            ...
            if (aboutMe && indexScroller && "IntersectionObserver" in window) {
                const io = new IntersectionObserver(
                    (entries) => {
                        nearAbout = !!(entries && entries[0] && entries[0].isIntersecting);
                        schedule();
            ...
            if (indexScroller) indexScroller.addEventListener("scroll", schedule, { passive: true });
